...
import com.gan.base.viewhelper.ViewDragHelper;
...
public class DragLayout extends FrameLayout {
	...
	public DragLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		dragHelper.setHasVertical(true);
	}
	...
	public float getSlideLeftWidthScale() {
		return scale;
	}
	 ...
	 *            默认0.75f
	 */
	public void setSlideLeftWidthScale(float scale) {
		this.scale = scale;
	}
	...
	protected void onFinishInflate() {
		...
		// 简单的异常处理
		if (getChildCount() != 2) {
			throw new IllegalArgumentException("DragLayout only have 2 children!");
		}
		vg_left = (LinearLayout) getChildAt(0);
		vg_main = (SlideLayout) getChildAt(1);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		range = (int) (width * scale);

		LayoutParams linearParams = (LayoutParams) vg_left.getLayoutParams();
		linearParams.width = range;
		vg_left.setLayoutParams(linearParams);
	}
